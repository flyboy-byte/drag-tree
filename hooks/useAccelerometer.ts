...
// 125 Hz target sample rate. Android 12+ requires the
// HIGH_SAMPLING_RATE_SENSORS permission for intervals < 200 ms (declared in
// app.json). Android may still cap this based on hardware; the diagnostic
// screen reports the actual achieved rate.
...
export function useAccelerometer({
  ...
  useEffect(() => {
    ...
    return () => sub.remove();
  }, [isAvailable, armed, watchForRedLight, sensitivity, onLaunch, onRedLight, onLaunchTelemetry]);
